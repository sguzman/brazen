...
impl RenderMode {
    fn from_value(value: &str) -> Self {
        value.parse().unwrap_or(Self::CpuReadback)
    }
}
...
impl FramePacing {
    fn from_value(value: &str) -> Self {
        value.parse().unwrap_or(Self::Vsync)
    }
}

impl FromStr for FramePacing {
    type Err = ();
...
impl ServoRuntimeConfig {
    pub fn from_engine_config(config: &EngineConfig) -> Self {
        Self {
            render_mode: RenderMode::from_value(&config.render_mode),
            webrender_backend: config.webrender_backend.clone(),
            frame_pacing: FramePacing::from_value(&config.frame_pacing),
        }
    }
}
